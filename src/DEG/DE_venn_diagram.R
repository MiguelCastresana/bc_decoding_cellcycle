...
colnames(comparaciones_combined) = trimws(colnames(comparaciones_combined),which = "left")
colnames(comparaciones_combined) = trimws(colnames(comparaciones_combined),which = "right")
...
fit <- euler(c("No phases" = 0, "With Phases" = 16, "No phases&With Phases" = 55))
plot(fit,
     fills = list(fill = c("#66CD00","dodgerblue"), alpha = 0.7),
